...
CREATE TABLE IF NOT EXISTS `train` (
  ...
  `tname` int(50) NOT NULL COMMENT 'Train_name',
  `sp` varchar(50) NOT NULL COMMENT 'Starting_Point',
...
-- Triggers `booking agent`
--
DROP TRIGGER IF EXISTS `before_insert_on_Booking_Agent`;
DELIMITER //
CREATE TRIGGER `before_insert_on_Booking_Agent` BEFORE INSERT ON `Booking_Agent`
 --FOR EACH ROW begin
...
//
DELIMITER ;
DROP TRIGGER IF EXISTS `before_update_on_Booking_Agent`;
DELIMITER //
CREATE TRIGGER `before_update_on_Booking_Agent` BEFORE UPDATE ON `Booking_Agent`
 --FOR EACH ROW begin
--if (year(curdate())-year(new.dob))<18 then 
--SIGNAL SQLSTATE '45000'
--SET MESSAGE_TEXT = 'Minimum age bar of 18 years.';
